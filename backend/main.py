...
COLOR_REFERENCE_DIR = BACKEND_DIR / "db" / "color_reference"
...
def init_database():
  ...
  # Create color_references table for color palette reference image entries
  cursor.execute("""
  ...
  """)
  cursor.execute("""
    CREATE INDEX IF NOT EXISTS idx_color_references_user_id
    ON color_references(user_id)
  """)
  cursor.execute("""
    CREATE INDEX IF NOT EXISTS idx_color_references_created_at
    ON color_references(created_at DESC)
  """)

  cursor.execute("PRAGMA table_info(color_references)")
  color_reference_columns = [row[1] for row in cursor.fetchall()]
  if "thumbnail_path" not in color_reference_columns:
    cursor.execute("ALTER TABLE color_references ADD COLUMN thumbnail_path TEXT")
  ...
  print(f"[info] Color reference directory at {COLOR_REFERENCE_DIR}")
...
def save_color_reference_file(file: UploadFile, user_id: str) -> Dict[str, Optional[str]]:
  if not file.filename:
    raise HTTPException(status_code=400, detail="Missing filename")
  ext = Path(file.filename).suffix.lower()
  if ext not in {".png", ".jpg", ".jpeg", ".webp", ".gif", ".bmp"}:
  ...
  safe_user_id = sanitize_user_id(user_id)
  user_dir = (COLOR_REFERENCE_DIR / safe_user_id).resolve()
  if not str(user_dir).startswith(str(COLOR_REFERENCE_DIR.resolve())):
    raise HTTPException(status_code=400, detail="Invalid color reference path")
  user_dir.mkdir(parents=True, exist_ok=True)
...
@app.get("/color-references")
def list_color_references(user: sqlite3.Row = Depends(require_current_user)):
  conn = get_db_connection()
  cursor = conn.cursor()
  cursor.execute("""
    SELECT id, original_name, file_path, thumbnail_path, mime_type, created_at
...
@app.post("/color-references/upload")
def upload_color_reference(file: UploadFile = File(...), user: sqlite3.Row = Depends(require_current_user)):
  saved = save_color_reference_file(file, user["id"])
  reference_id = str(uuid.uuid4())
  conn = get_db_connection()
  cursor = conn.cursor()
  cursor.execute("""
    INSERT INTO color_references (id, user_id, original_name, file_path, thumbnail_path, mime_type)
    VALUES (?, ?, ?, ?, ?, ?)
  """, (
    reference_id,
    user["id"],
...
@app.delete("/color-references/{reference_id}")
def delete_color_reference(reference_id: str, user: sqlite3.Row = Depends(require_current_user)):
  conn = get_db_connection()
  cursor = conn.cursor()
  cursor.execute("""
    SELECT id, file_path, thumbnail_path FROM color_references WHERE id = ? AND user_id = ?
  """, (reference_id, user["id"]))
  row = cursor.fetchone()
  if not row:
    conn.close()
    raise HTTPException(status_code=404, detail="Color reference not found")
  file_path = row["file_path"]
  thumbnail_path = row["thumbnail_path"]
  conn.execute("""
    DELETE FROM color_references WHERE id = ? AND user_id = ?
  """, (reference_id, user["id"]))
  conn.commit()
  conn.close()
  ...
  safe_user_id = sanitize_user_id(user["id"])
  user_dir = (COLOR_REFERENCE_DIR / safe_user_id).resolve()
  if str(user_dir).startswith(str(COLOR_REFERENCE_DIR.resolve())):
    stored_file = (user_dir / Path(file_path).name).resolve()
    if str(stored_file).startswith(str(user_dir)) and stored_file.exists() and stored_file.is_file():
      try:
        stored_file.unlink()
...
@app.get("/color-reference/{username}/{filename}")
def get_color_reference_file(username: str, filename: str):
  safe_username = sanitize_user_id(username)
  if safe_username != username:
    raise HTTPException(status_code=404, detail="Color reference file not found")
  file_path = (COLOR_REFERENCE_DIR / safe_username / filename).resolve()
  if not str(file_path).startswith(str(COLOR_REFERENCE_DIR.resolve())):
    raise HTTPException(status_code=400, detail="Invalid color reference path")
  if not file_path.exists() or not file_path.is_file():
    raise HTTPException(status_code=404, detail="Color reference file not found")
  return FileResponse(file_path)
